...
describe('SortProperties', () => {
  ...
  describe('initialSortProperty', () => {
    test('is set', () => {
      const sortableProperties = new _sortable_properties.SortableProperties([name, size, color], 'size');
      expect(sortableProperties.getSortedProperty().name).toBe('size');
    });

    test('throws an error with an invalid property name', () => {
      expect(() => new _sortable_properties.SortableProperties([name, size, color], 'does not exist')).toThrow();
    });

    test('throws an error property name is not defined', () => {
      expect(() => new _sortable_properties.SortableProperties([name, size, color])).toThrow();
    });
  });
  ...
  describe('isAscendingByName', () => {
    test('returns initial ascending values', () => {
      ...
      const sortableProperties = new _sortable_properties.SortableProperties([name, size, color], 'color');
  ...
  describe('sortItems', () => {
    test('sorts by initialSortProperty', () => {
      const sortableProperties = new _sortable_properties.SortableProperties([name, size, color], 'name');
