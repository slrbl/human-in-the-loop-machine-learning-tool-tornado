...
const ENABLE_INSTRUCTIONS = exports.ENABLE_INSTRUCTIONS = {
  OSX: {
    ...
    commands: ['./metricbeat modules enable redis'],
    textPost: 'Modify the settings in the `modules.d/redis.yml` file.'
  },
  DEB: {
    title: 'Enable and configure the redis module',
    commands: ['sudo metricbeat modules enable redis'],
  ...
  WINDOWS: {
    ...
    commands: ['PS C:\\Program Files\\Metricbeat> metricbeat.exe modules enable redis'],
    textPost: 'Modify the settings in the `modules.d/redis.yml` file.'
  }
};
