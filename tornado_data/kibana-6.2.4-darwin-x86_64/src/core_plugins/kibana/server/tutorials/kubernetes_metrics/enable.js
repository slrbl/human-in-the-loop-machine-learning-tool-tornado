...
const ENABLE_INSTRUCTIONS = exports.ENABLE_INSTRUCTIONS = {
  OSX: {
    ...
    commands: ['./metricbeat modules enable kubernetes'],
    textPost: 'Modify the settings in the `modules.d/kubernetes.yml` file.'
  },
  DEB: {
    title: 'Enable and configure the kubernetes module',
    commands: ['sudo metricbeat modules enable kubernetes'],
  ...
  WINDOWS: {
    ...
    commands: ['PS C:\\Program Files\\Metricbeat> metricbeat.exe modules enable kubernetes'],
    textPost: 'Modify the settings in the `modules.d/kubernetes.yml` file.'
  }
};
