...
const ENABLE_INSTRUCTIONS = exports.ENABLE_INSTRUCTIONS = {
  OSX: {
    ...
    commands: ['./filebeat modules enable mysql'],
    textPost: 'Modify the settings in the `modules.d/mysql.yml` file.'
  },
  DEB: {
    title: 'Enable and configure the mysql module',
    commands: ['sudo filebeat modules enable mysql'],
  ...
  WINDOWS: {
    ...
    commands: ['PS C:\\Program Files\\Filebeat> filebeat.exe modules enable mysql'],
    textPost: 'Modify the settings in the `modules.d/mysql.yml` file.'
  }
};
