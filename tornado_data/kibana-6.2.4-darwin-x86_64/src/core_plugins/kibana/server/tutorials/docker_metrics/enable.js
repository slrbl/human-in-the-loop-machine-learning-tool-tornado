...
const ENABLE_INSTRUCTIONS = exports.ENABLE_INSTRUCTIONS = {
  OSX: {
    ...
    commands: ['./metricbeat modules enable docker'],
    textPost: 'Modify the settings in the `modules.d/docker.yml` file.'
  },
  DEB: {
    title: 'Enable and configure the docker module',
    commands: ['sudo metricbeat modules enable docker'],
  ...
  WINDOWS: {
    ...
    commands: ['PS C:\\Program Files\\Metricbeat> metricbeat.exe modules enable docker'],
    textPost: 'Modify the settings in the `modules.d/docker.yml` file.'
  }
};
