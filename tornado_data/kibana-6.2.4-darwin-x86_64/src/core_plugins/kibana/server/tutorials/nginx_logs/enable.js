...
const ENABLE_INSTRUCTIONS = exports.ENABLE_INSTRUCTIONS = {
  OSX: {
    ...
    commands: ['./filebeat modules enable nginx'],
    textPost: 'Modify the settings in the `modules.d/nginx.yml` file.'
  },
  DEB: {
    title: 'Enable and configure the nginx module',
    commands: ['sudo filebeat modules enable nginx'],
  ...
  WINDOWS: {
    ...
    commands: ['PS C:\\Program Files\\Filebeat> filebeat.exe modules enable nginx'],
    textPost: 'Modify the settings in the `modules.d/nginx.yml` file.'
  }
};
