...
const ENABLE_INSTRUCTIONS = exports.ENABLE_INSTRUCTIONS = {
  OSX: {
    ...
    commands: ['./filebeat modules enable redis'],
    textPost: 'Modify the settings in the `modules.d/redis.yml` file.'
  },
  DEB: {
    title: 'Enable and configure the redis module',
    commands: ['sudo filebeat modules enable redis'],
  ...
  WINDOWS: {
    ...
    commands: ['PS C:\\Program Files\\Filebeat> filebeat.exe modules enable redis'],
    textPost: 'Modify the settings in the `modules.d/redis.yml` file.'
  }
};
